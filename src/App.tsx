...
// Animated Waterfall Chart Component
function WaterfallChart() {
  ...
  const bars = [
    { label: t('pe.waterfall.bars.current'), value: 100, color: '#6B7280', isBase: true },
    { label: t('pe.waterfall.bars.automation'), value: 25, color: '#10B981', isPositive: true },
    { label: t('pe.waterfall.bars.ai'), value: 35, color: '#10B981', isPositive: true },
    { label: t('pe.waterfall.bars.fees'), value: -8, color: '#EF4444', isNegative: true },
    { label: t('pe.waterfall.bars.adjusted'), value: 152, color: '#436be0', isResult: true },
  ]
  ...
  const processedBars = bars.map((bar) => {
    ...
    if (bar.isBase) {
      ...
      return { ...bar, startY: 0, height: bar.value }
    } else if (bar.isResult) {
      return { ...bar, startY: 0, height: bar.value }
    } else {
      if (bar.isPositive) {
        const start = prevCumulative
        cumulative += bar.value
        return { ...bar, startY: start, height: bar.value }
      } else {
        cumulative += bar.value
        return { ...bar, startY: cumulative, height: Math.abs(bar.value) }
      }
    }
  })
  ...
  return (
    <div ref={ref} className="relative">
      <svg className="w-full" viewBox={`0 0 ${bars.length * (barWidth + gap) + gap} ${chartHeight + 80}`} preserveAspectRatio="xMidYMid meet">
        {/* Grid lines */}
        {[0, 40, 80, 120, 160].map((val) => (
          <g key={val}>
            ...
            <text
              x="-10"
              y={chartHeight - (val / maxValue) * chartHeight + 24}
              fill="white"
              fillOpacity="0.3"
              fontSize="10"
              textAnchor="end"
        ...
        {processedBars.map((bar, index) => {
          ...
          const barHeight = (bar.height / maxValue) * chartHeight
          const y = chartHeight - (bar.startY / maxValue) * chartHeight - barHeight + 20
          ...
          return (
            <g key={index}>
              ...
              <motion.rect
                ...
                transition={{ duration: 0.8, delay: 0.3 + index * 0.15, ease: easeOutExpo }}
                style={{ transformOrigin: `${x + barWidth/2}px ${chartHeight + 20}px` }}
              />
              ...
              >
                {bar.isNegative ? bar.value : `+${bar.value - (bar.isBase || bar.isResult ? bar.value : 0) || bar.value}`}
                {bar.isBase || bar.isResult ? '' : ''}
              </motion.text>

              {/* Connector lines for waterfall */}
              {!bar.isBase && !bar.isResult && index < bars.length - 1 && (
                <motion.line
                  x1={x + barWidth}
                  y1={chartHeight - (cumulative / maxValue) * chartHeight + 20}
                  x2={x + barWidth + gap}
                  y2={chartHeight - (cumulative / maxValue) * chartHeight + 20}
                  stroke="white"
                  strokeOpacity="0.2"
                  strokeDasharray="4 4"
                  initial={{ pathLength: 0 }}
              ...
              >
                {bar.label.split(' ').map((word, i) => (
                  <tspan key={i} x={x + barWidth / 2} dy={i === 0 ? 0 : 12}>{word}</tspan>
                ))}
              </motion.text>
            </g>
          )
